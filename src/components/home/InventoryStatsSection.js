  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // DEBUG — temporary, safe to delete once live stats are verified.
        console.log("Amber Response:", err);
        if (cancelled) return;
        scheduleRetry();
        return;
      }
      ...
      // DEBUG — temporary, safe to delete once live stats are verified.
      // No raw "Inventory Array" is logged here: totals now come straight
      // from Amber's `meta.count` via its own `available` filter (two
      // count-only requests), not a client-side array fetch/filter — see
      // api/_lib/inventoryStats.js for why that's the correct approach
      // against Amber's real, paginated API.
      console.log("Amber Response:", response);
      console.log("Total:", response?.total);
      console.log("Sold Out:", response?.soldOut);

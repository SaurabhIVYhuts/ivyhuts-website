// Backend (CommonJS) twin of src/lib/googleMapsParser.js — same host
// allow-list, same extraction rules, same coordinate validation, kept as an
// exact structural mirror on purpose (same duplication precedent as
// api/_lib/campusUniversityResolver.js vs its frontend twin). This is the
// AUTHORITATIVE parse: api/universities/resolve.js always re-parses the raw
// `q` string itself server-side rather than trusting anything the client
// precomputed — the client-side parser only exists for instant UX branching
// (see UniversitySearchBox.js) without a wasted round trip.
//
...
const { hasValidCoords } = require("./universityDiscovery");
...
// institution-name character survives untouched.
...
// Same return shape as the frontend twin — see src/lib/googleMapsParser.js
...
function parseGoogleMapsUrl(input) {
    ...
    const confidence = coords && coords.coordSource === "precise" && name ? "high" : coords || name ? "medium" : "low";
